...
# -------------------- Fontes --------------------
try:
    fonte = pygame.font.Font('OMORI_GAME2.ttf', 36)
    fonte_pequena = pygame.font.Font('OMORI_GAME2.ttf', 24)
except Exception:
...
class PainelStatus:
    ...
    def desenhar(self, tela, fundo, imagem_atual):
        if self.imagem_fundo:
            tela.blit(self.imagem_fundo, (self.x + 15, self.y))
        else:
        ...
        tela.blit(imagem_atual, (self.x + 25, self.y + 10))
...
def iniciar_batalha():
    monstros = [
        Monstro("Lobo Terrivel", random.randint(50, 480), random.randint(10, 50), imagem=imagem_lobo),
        Monstro("Gatesco", random.randint(50, 480), random.randint(10, 50), imagem=imagem_gato),
        Monstro("O Sol", random.randint(50, 480), random.randint(10, 50), imagem=imagem_sol, background_img=fundo_sol),
        Monstro("Macaco No Abismo", random.randint(50, 480), random.randint(10, 50), imagem=imagem_macaco),
        Monstro("Slime", random.randint(50, 480), random.randint(10, 50), imagem=imagem_slime),
    ]
...
if imagem_botao_lutar:
    botao_atacar = Botao(276, 500, 300, 50, imagem=imagem_botao_lutar)
else:
    botao_atacar = Botao(276, 500, 300, 50, "Lutar", VERMELHO, BRANCO)

if imagem_botao_fugir:
    botao_fugir = Botao(275, 550, 300, 50, imagem=imagem_botao_fugir)
else:
    botao_fugir = Botao(275, 550, 300, 50, "Fugir", AZUL, BRANCO)

botao_espada = Botao(275, 500, 150, 50, "Espada", AZUL, BRANCO)
botao_magia = Botao(425, 500, 150, 50, "Magia", VERDE, BRANCO)
botao_voltar = Botao(425, 550, 150, 50, "Voltar", CINZA_CLARO, PRETO)
botao_descansar = Botao(275, 550, 150, 50, "Descansar", AMARELO_OURO, BRANCO)
            ...
            if estado_do_jogo == "menu_inicial":
                ...
                for carta in cartas_personagens:
                    if carta.is_clicado(pos):
                        ...
                        painel_jogador = PainelStatus(LARGURA_TELA//32, int(ALTURA_TELA//1.5), jogador.nome, jogador.vida, jogador.mana, jogador.vida_max, jogador.mana_max, imagem_painel_fundo)
